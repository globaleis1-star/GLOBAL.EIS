...
const WorldMap: React.FC<WorldMapProps> = ({
  ...
  return (
    <div ref={containerRef} className="relative w-full aspect-[16/10] bg-slate-50 dark:bg-slate-950 rounded-3xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-inner group">
      <svg
        viewBox="0 0 800 500"
      ...
      >
        ...
        <g className="labels pointer-events-none">
          {geographies.map((geo, i) => {
            ...
            // Only show labels for countries that have a reasonable area to avoid clutter
            // or if they are currently hovered/selected
            const countryName = geo.properties.name;
            const isSelected = isOrigin(geo) || isDestination(geo);
            const isHovered = hoveredCountry === countryName;
            
            // Simple heuristic: only show labels for countries with larger areas 
            // unless they are hovered or selected
